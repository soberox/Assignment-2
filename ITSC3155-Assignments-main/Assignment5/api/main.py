...
@app.post("/recipes/", response_model=schemas.OrderDetail, tags=["Recipes"])
def create_recipe(recipe: schemas.OrderDetailCreate, db: Session = Depends(get_db)):
    return recipes.create(db=db, recipe=recipe)


@app.get("/recipes/", response_model=list[schemas.OrderDetail], tags=["Recipes"])
def read_recipes(db: Session = Depends(get_db)):
    return recipes.read_all(db)


@app.get("/recipes/{recipe_id}", response_model=schemas.OrderDetail, tags=["Recipes"])
...
@app.put("/recipes/{recipe_id}", response_model=schemas.OrderDetail, tags=["Recipes"])
def update_one_recipe(recipe_id: int, recipe: schemas.OrderDetailUpdate, db: Session = Depends(get_db)):
...
@app.get("/orderdetails/{orderdetail_id}", response_model=schemas.OrderDetail, tags=["OrderDetails"])
...
def update_one_orderdetail(orderdetail_id: int, orderdetail: schemas.OrderDetailUpdate, db: Session = Depends(get_db)):
    orderdetail_db = orderdetails.read_one(db, orderdetail_id=orderdetail_id)
    if orderdetail_db is None:
        raise HTTPException(status_code=404, detail="User not found")
    return orderdetails.update(db=db, orderdetail=orderdetail, orderdetail_id=orderdetail_id)
...
def delete_one_orderdetail(orderdetail_id: int, db: Session = Depends(get_db)):
    ...
    if orderdetail is None:
        raise HTTPException(status_code=404, detail="User not found")
    return orderdetails.delete(db=db, orderdetail_id=orderdetail_id)